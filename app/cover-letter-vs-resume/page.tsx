...
export default function CoverLetterVsResumePage() {
  ...
  useEffect(() => {
    document.title = 'Cover Letter vs Resume: Do You Need Both in 2026? | ResumeBuildz';
    const desc = document.querySelector('meta[name="description"]');
    const copy = 'Do you still need a cover letter in 2026? Honest answer with data. When to write one, when to skip, and the hidden cost of going without.';
    if (desc) desc.setAttribute('content', copy);
    const ogDesc = document.querySelector('meta[property="og:description"]');
    if (ogDesc) ogDesc.setAttribute('content', copy);
    const ogTitle = document.querySelector('meta[property="og:title"]');
    if (ogTitle) ogTitle.setAttribute('content', 'Cover Letter vs Resume: Do You Need Both in 2026? | ResumeBuildz');
  }, []);
