...
export default function ResumeLengthPage() {
  ...
  useEffect(() => {
    document.title = 'Resume Length in 2026: 1 Page vs 2 Pages (Definitive Guide) | ResumeBuildz';
    const desc = document.querySelector('meta[name="description"]');
    const copy = 'How long should a resume be in 2026? 1 page under 5 years experience, 2 pages above. Full decision framework with career stage, industry, and cutting/expanding tactics.';
    if (desc) desc.setAttribute('content', copy);
    const ogDesc = document.querySelector('meta[property="og:description"]');
    if (ogDesc) ogDesc.setAttribute('content', copy);
    const ogTitle = document.querySelector('meta[property="og:title"]');
    if (ogTitle) ogTitle.setAttribute('content', 'Resume Length in 2026: 1 Page vs 2 Pages | ResumeBuildz');
  }, []);
