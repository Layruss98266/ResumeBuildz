...
export default function TailorResumePage() {
  ...
  useEffect(() => {
    document.title = 'How to Tailor Your Resume in 10 Minutes (Without Starting Over) | ResumeBuildz';
    const desc = document.querySelector('meta[name="description"]');
    const copy = 'Tailored resumes get 3x more callbacks. The minute-by-minute process to customize your resume for any JD without starting from scratch. Plus the AI-assisted workflow.';
    if (desc) desc.setAttribute('content', copy);
    const ogDesc = document.querySelector('meta[property="og:description"]');
    if (ogDesc) ogDesc.setAttribute('content', copy);
    const ogTitle = document.querySelector('meta[property="og:title"]');
    if (ogTitle) ogTitle.setAttribute('content', 'How to Tailor Your Resume in 10 Minutes | ResumeBuildz');
  }, []);
