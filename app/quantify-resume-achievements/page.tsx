...
export default function QuantifyAchievementsPage() {
  ...
  useEffect(() => {
    document.title = 'How to Quantify Resume Achievements (50+ Examples by Role) | ResumeBuildz';
    const desc = document.querySelector('meta[name="description"]');
    const copy = 'Turn vague bullets into metric-driven proof. The XYZ formula, where to find numbers, and 50+ quantified bullet examples across engineering, sales, marketing, ops, finance, healthcare, education, retail.';
    if (desc) desc.setAttribute('content', copy);
    const ogDesc = document.querySelector('meta[property="og:description"]');
    if (ogDesc) ogDesc.setAttribute('content', copy);
    const ogTitle = document.querySelector('meta[property="og:title"]');
    if (ogTitle) ogTitle.setAttribute('content', 'How to Quantify Resume Achievements (50+ Examples) | ResumeBuildz');
  }, []);
