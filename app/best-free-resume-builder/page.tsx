...
export default function BestFreeResumeBuilderPage() {
  ...
  useEffect(() => {
    document.title = 'Free vs Paid Resume Builders 2026: Brutally Honest Comparison | ResumeBuildz';
    const desc = document.querySelector('meta[name="description"]');
    const copy = '8 resume builders compared honestly: pricing, privacy, ATS compatibility, AI features. Which "free" builders paywall the download. Open source options.';
    if (desc) desc.setAttribute('content', copy);
    const ogDesc = document.querySelector('meta[property="og:description"]');
    if (ogDesc) ogDesc.setAttribute('content', copy);
    const ogTitle = document.querySelector('meta[property="og:title"]');
    if (ogTitle) ogTitle.setAttribute('content', 'Free vs Paid Resume Builders 2026: Honest Comparison | ResumeBuildz');
  }, []);
