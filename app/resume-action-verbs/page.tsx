...
export default function ResumeActionVerbsPage() {
  ...
  useEffect(() => {
    document.title = '200+ Resume Action Verbs by Category (with Examples) | ResumeBuildz';
    const desc = document.querySelector('meta[name="description"]');
    const copy = '210 powerful action verbs grouped by role (leadership, technical, sales, analysis, more). Includes the weak-to-strong swap table and 10 real bullet examples.';
    if (desc) desc.setAttribute('content', copy);
    const ogDesc = document.querySelector('meta[property="og:description"]');
    if (ogDesc) ogDesc.setAttribute('content', copy);
    const ogTitle = document.querySelector('meta[property="og:title"]');
    if (ogTitle) ogTitle.setAttribute('content', '200+ Resume Action Verbs by Category (with Examples) | ResumeBuildz');
  }, []);
