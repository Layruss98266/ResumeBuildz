...
export default function ResumeFormatPage() {
  ...
  useEffect(() => {
    document.title = 'Best Resume Format in 2026: Chronological vs Functional vs Hybrid | ResumeBuildz';
    const desc = document.querySelector('meta[name="description"]');
    const copy = 'The 3 resume formats compared: reverse-chronological, functional, and hybrid. Which beats ATS, which recruiters actually read, and when each one fails.';
    if (desc) desc.setAttribute('content', copy);
    const ogDesc = document.querySelector('meta[property="og:description"]');
    if (ogDesc) ogDesc.setAttribute('content', copy);
    const ogTitle = document.querySelector('meta[property="og:title"]');
    if (ogTitle) ogTitle.setAttribute('content', 'Best Resume Format in 2026: Chrono vs Functional vs Hybrid | ResumeBuildz');
  }, []);
